...
def preprocess_input(input_dict):
    ...
    for key, value in input_dict.items():
        ...
        df['cf_ab_score'] = round(df['consume_frequency(weekly)'] / (df['consume_frequency(weekly)']
                            + df['awareness_of_other_brands']), 2)
        df['zas_score'] = df['zone'] * df['income_levels']
        df['bsi'] = 1 if (input_dict['Current Brand'] != 'Established' and
                    (input_dict['Reasons for choosing brands'] in ['Price', 'Quality'])) else 0
